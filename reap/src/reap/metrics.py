...
def ca_dist_online(
    ...
    # Optimization: for cosine and angular distances, use bmm to avoid (S, E, E, H) broadcasting
    # which is extremely memory intensive.
    if distance_callable in [cosine_distance, angular_distance]:
        # Normalize along hidden_dim
        normed = F.normalize(permuted_activations, p=2, dim=-1) # (S, E, H)
        # Compute pairwise inner products using batimport numpy as np
import torch
import torch.nn.functional as F
import math  # For pi constant in comments
from typing import List, Dict, Callable
...
def ca_dist_online(
    ...
    if distance_callable in [cosine_distance, angular_distance]:
        # Normalize along hidden_dim
        normed = F.normalize(permuted_activations, p=2, dim=-1) # (S, E, H)
        # Compute pairwise inner products using batch matrix multiplication: (S, E, H) @ (S, H, E) -> (S, E, E)
        similarities = torch.bmm(normed, normed.transpose(1, 2)) # (S, E, E)
        similarity_matrix = similarities.mean(dim=0) # (E, E)
        
        if distance_callable == cosine_distance:
            return 1.0 - similarity_matrix
        else: # angular_distance
            clamped_similarity = torch.clamp(similarity_matrix, -1.0, 1.0)
            angle = torch.acos(clamped_similarity)
            return angle / math.pi

    # Fallback to original implementation for other distance metrics
...
class OnlineStatsTracker:
    ...
    def merge(self, other: "OnlineStatsTracker"):
        ...
        if isinstance(new_count, torch.Tensor):
            ...
            self.count = new_count
ch matrix multiplication: (S, E, H) @ (S, H, E) -> (S, E, E)
        similarities = torch.bmm(normed, normed.transpose(1, 2)) # (S, E, E)
        similarity_matrix = similarities.mean(dim=0) # (E, E)
